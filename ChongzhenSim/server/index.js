...
function buildUserMessage(body) {
  ...
  const expectedTime = `崇祯${year}年${month}月 ${phaseLabel}`;
  ...
  const prestige = state.prestige ?? 60;
  const executionRate = state.executionRate ?? 75;
  
  const nationStr = `国库=${treasury.toLocaleString()}两（${treasuryStatus}）, 粮储=${grain.toLocaleString()}石（${grainStatus}）, 军力=${militaryStrength}, 民心=${civilMorale}（${moraleStatus}）, 边患=${borderThreat}（${borderStatus}）, 天灾=${disasterLevel}, 贪腐=${corruptionLevel}（${corruptionStatus}）, 威望=${prestige}, 政令执行率≈${executionRate}%`;

  let base = "";
  if (lastChoiceId == null || lastChoiceText == null) {
    base = `当前是${expectedTime}。国势：${nationStr}。这是新开档第一回合，请生成【${expectedTime}】的完整剧情与 3 个选项。lastChoiceEffects 设为 null。只输出上述 JSON，不要其他内容。\n【重要】header.time 必须严格等于 \"${expectedTime}\"。`;
  } else {
    ...
    base = `当前是${expectedTime}。国势：${nationStr}。上一回合陛下选择了：id=${lastChoiceId}，文案="${lastChoiceText}"。
${effectHint}
【重要】剧情必须根据当前国势合理推演！如果国库充裕，不能写"国库空虚"；如果民心归附，不能写"民怨沸腾"。
  ...
  if (Array.isArray(currentQuarterAgenda) && currentQuarterAgenda.length > 0) {
    const agendaText = currentQuarterAgenda.map((item, idx) => `${idx + 1}. ${item.title}：${item.summary}`).join("\n");
    base += `\n\n本季度核心奏折议题如下（季度月必须围绕其一展开）：\n${agendaText}`;
  }

  if (currentQuarterFocus && currentQuarterFocus.agendaId) {
    base += `\n\n本回合玩家已锁定季度议题：${JSON.stringify(currentQuarterFocus)}`;
  }

  if (playerAbilities && typeof playerAbilities === "object") {
    base += `\n\n皇帝能力加点：${JSON.stringify(playerAbilities)}`;
  }

  if (Array.isArray(unlockedPolicies) && unlockedPolicies.length > 0) {
    base += `\n\n已实施国策：${unlockedPolicies.join("、")}`;
  }
...
app.post("/api/chongzhen/ministerChat", async (req, res) => {
  ...
  const agendaHint = currentQuarterFocus && currentQuarterFocus.agendaId
    ? `\n当前季度焦点：${JSON.stringify(currentQuarterFocus)}\n可选季度议题：${JSON.stringify(currentQuarterAgenda.slice(0, 5))}`
    : "";
...
- 根据大臣立场和陛下的话语做出合理回应，可以有不同意见
 - 若当前季度焦点与本大臣派系/立场一致，回复中应体现更强执行意愿；若冲突，则提出具体顾虑
