...
- 玩法兼容与修复说明：
  ...
  - 系统提示词新增硬约束：剧情、选项、提示必须为中文，不得出现英文国策 ID。
...
#### 6.1.24 492c4e9 · fix(gameplay): tighten hostile trigger and dedupe quarter delta panel
- 全哈希：`492c4e9fc1ac7a88de1d2a941eaf6fb5027ac3f4`
- 时间：2026-03-22
- 分支：my-feature-branch
- 作者：JINTIAN-LIU
- 类型：fix
- 变更文件：
  ...
  - ChongzhenSim/js/systems/storySystem.js
- 玩法兼容与修复说明：
  - 修复敌对势力被误触发打击的问题，只有存在明确军事意图时才会从文本中推断 hostile 目标；非军事叙述即使提到敌对势力，也不会触发打击。
  - 季度月诏书面板改为只展示一次合并后的季度结算数值变化，避免与本轮推演面板重复显示。
  - 季度结算展示链路补足去重合并逻辑，避免同一数值变化在历史面板中重复追加。
- 自检结果：
...
- 变更文件：
  ...
  - ChongzhenSim/server/index.test.js
- 玩法兼容与修复说明：
  - 修复季度月面板运行时报错，补回 `DISPLAY_STATE_METRICS` 导入，解决 `storySystem.js` 中的运行时异常。
  - 新增 `appointmentEffects` 语义解析模块，将“任命 / 罢免 / 免去 / 改任”等诏书文本解析为结构化官职任免效果并在回合链路中落地。
  - `buildUserMessage` 的大臣官职列表改为基于当前 `appointments` 动态生成，保证手动调官后的朝堂状态会进入下一轮诏书推理上下文。
  - `storySystem.applyEffects` 新增 `appointmentDismissals` 处理，确保免职效果能正确清空对应官位。
- 自检结果：
...
- 变更文件：
  ...
  - ChongzhenSim/js/utils/appointmentEffects.test.js
- 玩法兼容与修复说明：
  - 修复诏书中任命已有官职时不生效的问题，将职位名和人物名统一归一化映射到 canonical `positionId` / `characterId`，避免 LLM 输出中文称谓时映射失败。
  - `turnSystem` 在应用任免效果前统一执行 `normalizeAppointmentEffects`，确保整条诏书链路始终以 canonical id 落地。
  - `storySystem.applyEffects` 同步增加标准化兜底，兼容历史分支和回放链路。
  - 初始化状态新增 `positionsMeta` 挂载，为前端展示和标准化逻辑提供稳定职位元数据。
- 追加测试：
  - `appointmentEffects.test.js` 新增职位名 / 人物名映射、免职列表映射等标准化测试。
- 自检结果：
...
- 变更文件：
  ...
  - ChongzhenSim/js/systems/coreGameplaySystem.test.js
- 玩法兼容与修复说明：
  - 修复诏书执行率缩放链路会丢弃 `appointments`、`appointmentDismissals`、`characterDeath` 等非数值效果字段的问题，避免任免结果被执行缩放吞掉。
  - `scaleEffectsByExecution` 改为仅缩放数值字段，完整保留结构化效果字段，确保官职状态变更稳定落地。
- 追加测试：
  - `coreGameplaySystem.test.js` 新增回归用例，验证 execution 缩放后仍保留 `appointments`、`appointmentDismissals`、`characterDeath` 等字段。
- 自检结果：
...
- 变更文件：
  ...
  - ChongzhenSim/js/utils/displayStateMetrics.test.js
- 玩法兼容与修复说明：
  - 诏书数值面板中的任命 / 免职展示改为优先使用职位中文名，避免出现职位 ID 拼音字符串。
